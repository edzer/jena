...
## What is wrong with R's native `array`?

- nothing!!
- dimensions can be named, dimension values labeled:
```{r echo=TRUE}
array(1:8/8, c(2,2,2), dimnames = list(rows=c("a","b"), cols=c("p", "q"), 
     layers=c("x", "y")))[,,1,drop=FALSE]
```
...
For me, `stars` was a blueprint, an exercise to answer parts of the question _how can we do this?_
